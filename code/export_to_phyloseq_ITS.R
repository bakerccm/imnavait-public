...
# get file names from command line arguments
    ...
    filenames <- list(
        ...
        input_file_metadata = args[3], # sample metadata file
...
# get sample metadata and parse
    ...
    # add Month.Depth_range interaction
        sample.metadata$Month.Depth_range <- paste(sample.metadata$Month, sample.metadata$Depth_range, sep=".")

    # make month a factor to retain order
        sample.metadata$Month <- factor(sample.metadata$Month, levels=c("June", "August", "October"))
